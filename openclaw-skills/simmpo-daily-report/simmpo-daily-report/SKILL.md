...
- **Shopline 營業額**：依地區（TW/HK/MO/MY）彙整指定日期的訂單金額，排除 `cancelled` 訂單
...
| `csv_parser.py` | CSV 解析：找目標日期列、取花費/購買/購買轉換值 |
...
```
請在 simmpo-claw 上執行以下指令，並將腳本的原始輸出完整回傳（不要重新計算或改寫數字）：

ssh simmpo-claw "export PATH=\$HOME/local/bin:\$PATH && python3 ~/services/daily_report.py \
  --date 2026-03-23 \
  --tokens ~/services/simmpo_tokens.json \
  --agent-browser 'npx agent-browser' 2>&1"
```

> **注意**：`npx agent-browser` 需要 `~/local/bin` 在 PATH 中，必須在 ssh 指令內明確加上 `export PATH=\$HOME/local/bin:\$PATH`。
...
| 渠道 | 資料來源 | 備註 |
|------|----------|------|
| 官網 TW 營業額 | Shopline TW | 跨境 7-11 取貨訂單歸入 TW |
| 官網 TW 廣告費 | Meta act_1481816722260744 | 僅台灣官網投放 |
| 官網 HK/MO 營業額 | Shopline HK/MO | 按收貨地址分類 |
...
- 肖準 Looker Studio：✅ v0.22 穩定（DateRangePicker 跨月切換，2/1、3/1、4/1 驗證通過）
