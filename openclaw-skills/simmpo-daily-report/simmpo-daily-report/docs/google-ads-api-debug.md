...
| 2026-04-01 | Google 回覆（Swathi）確認字元問題：第8位須為大寫 `I` |
| 2026-04-01 | 修正 simmpo_tokens.json developer_token，API 測試成功 ✅ |

## ✅ 解決方案（2026-04-01 Google 回覆）

**根本原因：字元混淆**

| 位置 | 字元 | 說明 |
|------|------|------|
| 原始嘗試 | `_cRipdml1...` | 第8位是小寫 `l`（L），錯誤 |
| 第二次嘗試 | `_cRipdmi1...` | 第8位改成小寫 `i`，仍錯誤 |
| **正確值** | `_cRipdmI1...` | 第8位必須是大寫 `I`（i 的大寫），正確 ✓ |

Google Case #0-9454000041255 由 Swathi（gTech）回覆確認。

**行動：** 將所有使用 developer token 的地方更新為大寫 `I` 的版本。

## 下一步

- [x] 等到 3/23（核准後 5 個工作天）再測試
- [x] 等待 Google Case #25327540079 回覆 → 2026-04-01 收到回覆
- [x] 確認字元問題 → 已確認是 `I`（大寫i）
- [x] 更新 simmpo_tokens.json 中的 developer token 為正確值（大寫 `I`）
- [x] 測試 API call 確認解決 → `Simmpo-MCC帳戶` 回傳成功
